...
[
  {"question": "...", "options": ["A","B","C","D"], "answer": "A"}
      ...
      if (!Array.isArray(parsed) || parsed.length === 0) {
        ...
        parsed = [
          { question: "Placeholder question", options: ["A", "B", "C", "D"], answer: "A" }
        ];
      }
      ...
      const normalized = parsed.map((q) => {
        ...
        if (!Array.isArray(options)) options = typeof options === "string" ? options.split("\n").map(s=>s.trim()).filter(Boolean) : [];
        while (options.length < 4) options.push(`Option ${options.length + 1}`);
        let answer = (q.answer || q.correct || "").toString().trim();
        if (!/^[A-D]$/i.test(answer)) answer = "A";
        return { question, options: options.slice(0,4), answer: answer.toUpperCase() };
      });
  ...
  const handleChoose = (opt) => {
    ...
    const isCorrect = selectedLetter === correctLetter || (opt && opt.toLowerCase() === (quiz[currentIndex].answer || "").toLowerCase());
    ...
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-white flex items-center justify-center p-6">
      <div className="w-full max-w-3xl bg-white shadow-lg rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-xl font-bold text-indigo-700">StudyFlow — Daily Quiz</h1>
          <div className="text-sm text-gray-500">Keep your streak alive ✨</div>
        </div>

        {step === 1 && (
          <div className="text-center py-10">
            <h2 className="text-2xl font-semibold mb-3">Did you complete today’s plan?</h2>
            <div className="flex justify-center gap-4">
              <button onClick={() => { setCheckedResponse("yes"); setStep(2); }} className="px-6 py-3 bg-green-500 text-white rounded-lg">Yes ✅</button>
              <button onClick={() => { setCheckedResponse("no"); setStep(2); }} className="px-6 py-3 bg-yellow-500 text-white rounded-lg">Not yet 💪</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="py-6">
            <div className={`p-4 rounded-lg mb-6 ${checkedResponse === "yes" ? "bg-green-50 border border-green-200" : "bg-yellow-50 border border-yellow-200"}`}>
              <h3 className="font-semibold mb-2">{checkedResponse === "yes" ? "Awesome — keep it up!" : "No worries — small steps matter!"}</h3>
              <p className="text-gray-700">{checkedResponse === "yes" ? "You did great today. Let’s test what you learned!" : "Let’s do a short quiz to build momentum — you got this!"}</p>
            </div>

            <div className="mb-4">
              <label className="block text-sm font-medium mb-2">Select Class</label>
              <select value={studentClass} onChange={(e) => { setStudentClass(e.target.value); setSubjects([]); setChapters({}); }} className="w-full border rounded-md p-2">
                <option value="">Choose Class</option>
                {Object.keys(classData).map((c) => <option key={c} value={c}>Class {c}</option>)}
              </select>
            </div>

            {studentClass && (
              <div className="mb-4">
                <label className="block text-sm font-medium mb-2">Select Subjects (max 3)</label>
                <div className="flex flex-wrap gap-3">
                  {classData[studentClass].subjects.map((s) => (
                    <button key={s} type="button" onClick={() => toggleSubject(s)} className={`px-3 py-2 rounded-md border ${subjects.includes(s) ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-800"}`}>
                      {s}
                    </button>
                  ))}
                </div>
                <p className="text-xs text-gray-500 mt-2">Tip: choose up to 3 subjects to generate a focused quiz.</p>
              </div>
            )}

            {subjects.length > 0 && (
              <div className="mb-4">
                <label className="block text-sm font-medium mb-2">Choose one chapter per selected subject</label>
                <div className="space-y-3">
                  {subjects.map((s) => (
                    <div key={s}>
                      <div className="font-medium mb-1">{s}</div>
                      <select value={chapters[s] || ""} onChange={(e) => handleChapterChange(s, e.target.value)} className="w-full border rounded-md p-2">
                        <option value="">Choose Chapter for {s}</option>
                        {classData[studentClass].chapters[s].map((ch) => <option key={ch} value={ch}>{ch}</option>)}
                      </select>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex items-center gap-3">
              <button onClick={generateQuizWithGemini} disabled={loading} className="px-5 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-60">
                {loading ? "Generating..." : "Generate Quiz"}
              </button>
              <button onClick={restartAll} className="px-4 py-2 border rounded-md">Cancel</button>
            </div>
        ...
        {step === 5 && quiz.length > 0 && (
          <div className="py-4">
            <div className="flex justify-between items-center mb-3">
              <div className="text-sm text-gray-600">Question {currentIndex + 1} / {quiz.length}</div>
              <div className="text-sm font-semibold text-indigo-700">Score: {score}</div>
            </div>

            <div className="bg-gray-50 p-5 rounded-lg">
              <div className="text-lg font-medium mb-4">{quiz[currentIndex]?.question}</div>
              ...
              <div className="grid grid-cols-1 gap-3">
                {(quiz[currentIndex]?.options || []).map((opt, idx) => {
                  ...
                  const correctLetter = (quiz[currentIndex].answer || "").toUpperCase();
                  const correctIdx = /^[A-D]$/.test(correctLetter) ? correctLetter.charCodeAt(0) - 65 : -1;
                  const correctOpt = correctIdx >= 0 ? quiz[currentIndex].options[correctIdx] : null;

                  let style = "bg-white hover:bg-indigo-50 border";
                  if (selectedOption) {
                    if (isSelected) {
                      style = (opt === correctOpt) ? "bg-green-100 border-green-400 text-green-800" : "bg-red-100 border-red-400 text-red-800";
                  ...
                  return (
                    <button key={idx} disabled={!!selectedOption} onClick={() => { setSelectedOption(opt); handleChoose(opt); }} className={`text-left p-3 rounded-md ${style}`}>
                      <div className="flex items-center gap-3">
                        <div className="w-6 h-6 flex items-center justify-center font-semibold text-sm">{letter}</div>
                        <div>{opt}</div>
                      </div>
                    </button>
                  );
        ...
        {step === 6 && (
          <div className="py-6 text-center">
            ...
            <p className="text-gray-700 mb-4">You scored <span className="font-semibold">{score}</span> / <span className="font-semibold">{quiz.length}</span>.</p>

            {saving ? (
              <p className="text-indigo-600">Saving your result...</p>
            ) : (
              <p className="text-green-600 font-medium">Saved to your progress ✅</p>
            )}

            <div className="mt-5 flex justify-center gap-3">
              <button onClick={() => { setStep(2); resetQuizState(); }} className="px-5 py-2 bg-indigo-600 text-white rounded-md">Take another quiz</button>
              <button onClick={restartAll} className="px-4 py-2 border rounded-md">Back home</button>
            </div>
          </div>
        )}
